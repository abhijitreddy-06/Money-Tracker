...
const DepositRecordsScreen = ({ navigation }) => {
    ...
    const fetchDepositRecords = async () => {
        try {
            ...
            const response = await axios.get(`${API_BASE}/api/deposit`, {
            ...
            console.log('Deposit fetch error:', error.response?.data || error.message);
            Alert.alert('Error', 'Failed to fetch deposit records.');
        } finally {
            setLoading(false);
            setRefreshing(false);
        }
    ...
    const renderRecordItem = ({ item, index }) => {
        const icon = '💵';
        const color = '#16a34a'; // green for deposits
        const typeColor = '#16a34a';
        const recordDate = item.deposit_date ? new Date(item.deposit_date).toLocaleDateString('en-US', {
    ...
    const ListEmptyComponent = () => (
        <View style={styles.emptyState}>
            ...
            <Text style={styles.emptySubtext}>
                Your deposit transactions will appear here once you add them
            </Text>
        </View>
    );
            ...
            <View style={styles.content}>
                <View style={styles.headerCard}>
                    <View style={styles.titleContainer}>
                        ...
                        <Text style={styles.subtitle}>
                            {depositRecords.length} total deposits
                        </Text>
                    </View>
                </View>
                ...
                {loading ? (
                    <View style={styles.loadingContainer}>
                        ...
                        <Text style={styles.loadingText}>Loading your deposit records...</Text>
                    </View>
                ) : (
                    <Animated.View style={[styles.recordsContainer, { opacity: fadeAnim }]}>
                        <FlatList
                            ...
                            keyExtractor={(item, index) => (item.deposit_id ? item.deposit_id.toString() : index.toString())}
...
export default DepositRecordsScreen;
